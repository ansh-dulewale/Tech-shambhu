  ...
  return (
    <div className="glass-card p-4">
      <h2 className="text-lg font-semibold mb-3 text-yellow-400 flex items-center gap-2">
        <span className="animate-glow">🌩️</span> God Mode
      </h2>

      {/* State Selector */}
      <div className="mb-3">
        <label className="text-xs text-gray-400 block mb-1">Target State</label>
        <select
          id="god-mode-state-select"
          value={selectedState}
          onChange={(e) => setSelectedState(e.target.value)}
          className="w-full px-3 py-2 rounded-lg bg-gray-800/80 border border-gray-700 text-white text-sm
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        <button
          ...
          className={`px-3 py-2 rounded-lg text-xs font-medium transition-all duration-300
            ${activePanel === "event"
              ? "bg-orange-500/30 border border-orange-400 text-orange-300 shadow-lg shadow-orange-500/20"
              : "bg-gray-800/60 border border-gray-700 text-gray-300 hover:border-orange-400/50 hover:text-orange-300"
            }`}
        >
          ⚡ Trigger Event
        </button>
        ...
        <button
          ...
          className={`px-3 py-2 rounded-lg text-xs font-medium transition-all duration-300
            ${activePanel === "block"
              ...
              : "bg-gray-800/60 border border-gray-700 text-gray-300 hover:border-red-400/50 hover:text-red-300"
            }`}
        >
          🚫 Block Trade
        </button>
        ...
        <button
          ...
          className={`px-3 py-2 rounded-lg text-xs font-medium transition-all duration-300
            ${activePanel === "aid"
              ...
              : "bg-gray-800/60 border border-gray-700 text-gray-300 hover:border-green-400/50 hover:text-green-300"
            }`}
        >
          🎁 Give Aid
        </button>
        ...
        <button
          ...
          className={`px-3 py-2 rounded-lg text-xs font-medium transition-all duration-300
            ${activePanel === "force"
              ? "bg-purple-500/30 border border-purple-400 text-purple-300 shadow-lg shadow-purple-500/20"
              : "bg-gray-800/60 border border-gray-700 text-gray-300 hover:border-purple-400/50 hover:text-purple-300"
            }`}
        >
          🎯 Force Action
        </button>
      ...
      {/* ⚡ Trigger Event Panel */}
      {activePanel === "event" && (
        <div className="bg-gray-900/50 rounded-lg p-3 border border-orange-400/20 space-y-2 animate-float">
          <p className="text-xs text-orange-300 font-medium">Choose event type:</p>
            ...
            {EVENT_TYPES.map((evt) => (
              <button
                ...
                className="px-2 py-1.5 rounded-md text-xs transition-all duration-200
                           bg-gray-800/60 border border-gray-700 hover:border-orange-400/40
      ...
      {/* 🚫 Block Trade Panel */}
      {activePanel === "block" && (
        <div className="bg-gray-900/50 rounded-lg p-3 border border-red-400/20 space-y-2">
          <p className="text-xs text-red-300 font-medium">Block trade between:</p>
          <div className="flex gap-2 items-center">
            <select
              id="god-mode-block-state1"
              value={blockState1}
              onChange={(e) => setBlockState1(e.target.value)}
              className="flex-1 px-2 py-1.5 rounded-md bg-gray-800/80 border border-gray-700
                         text-white text-xs focus:outline-none focus:border-red-400/50"
            >
      ...
      {/* 🎁 Give Aid Panel */}
      {activePanel === "aid" && (
        <div className="bg-gray-900/50 rounded-lg p-3 border border-green-400/20 space-y-2">
          <p className="text-xs text-green-300 font-medium">Send aid to {selectedState || "..."}:</p>
            ...
            {RESOURCES.map((res) => (
              <button
                ...
                className={`flex-1 px-2 py-1.5 rounded-md text-xs transition-all
                  ${aidResource === res
                    ? "bg-green-500/20 border border-green-400/60 scale-105"
                    : "bg-gray-800/60 border border-gray-700 hover:border-green-400/30"
                  }`}
              >
          ...
          <div>
            <label className="text-xs text-gray-400">Amount: {aidAmount}</label>
            <input
      ...
      {/* 🎯 Force Action Panel */}
      {activePanel === "force" && (
        <div className="bg-gray-900/50 rounded-lg p-3 border border-purple-400/20 space-y-2">
            ...
            {ACTIONS.map((action) => (
              <button
                ...
                className={`px-2 py-1.5 rounded-md text-xs transition-all
                  ${forceAction === action
                    ? "bg-purple-500/20 border border-purple-400/60 text-purple-300 scale-105"
                    : "bg-gray-800/60 border border-gray-700 text-gray-400 hover:border-purple-400/30"
                  }`}
              >
