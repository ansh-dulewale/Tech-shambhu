  ...
  for (const [key, value] of Object.entries(effects)) {
    if (value !== 0) {
      ...
      badges.push(
        <span
          key={key}
          className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${isPositive ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'}`}
        >
  ...
  return (
    <div className="glass-card p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-xs text-gray-500 tabular-nums">{events.length} events</span>
      </div>
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    <span className="text-[11px] text-gray-500 font-mono tabular-nums">C{event.cycle}</span>
                    <span className="text-[11px] text-gray-700">|</span>
                    <span className="text-[11px] text-gray-400 uppercase tracking-wider">{event.stateId || 'Global'}</span>
                  </div>
